---
phase: 5
title: "Shopping Agent pipeline theo spec §20–29"
status: pending
priority: P1
effort: "1.5w"
dependencies: [1, 2]
...
## Requirements
- Functional:
  - `ShoppingIntent` **chuyển hẳn** sang schema §20 (D8): `{ category, subject: {childId?, childName?, weightKg?, diaperSize?, ageMonths?}, requirements: {nightUse?, leakProtection?, sensitiveSkin?}, constraints: {minPrice?, maxPrice?, brands?, excludedBrands?}, preferences: {price?, delivery?, preferredBrands?}, missingRequiredFields: string[] }`. Requirements riêng theo category qua discriminated union (`category: "diapers"` trước). Cập nhật mọi consumer (`recommend.ts`, `recommendation.ts`, `agent-shopping.tsx`, `evaluate.mjs`, persistence). Metadata intent cũ trong `messages.metadata` đọc qua hàm `upgradeIntent()` (không migrate DB). <!-- Updated: Validation Session 1 - D8 -->
  ...
  - Hỏi tối đa 1–3 câu khi thiếu slot bắt buộc (§7); đếm số câu hỏi lại trong hội thoại.
...
1b. Đổi `ShoppingIntent` sang dạng §20 + `upgradeIntent()`; typecheck kéo theo mọi consumer.
...
4. Ghi rejected products; cập nhật eval 50 case thêm kiểm tra excludedBrands và số câu hỏi lại ≤3.
