...
export async function DELETE() {
  ...
  const moneyErrors = await Promise.all(["family_notes", "purchases", "family_events", "money_transactions", "money_budgets", "money_recurring", "money_goals", "money_settings"].map(async (table) => (await client.from(table).delete().eq("user_id", userId)).error));
