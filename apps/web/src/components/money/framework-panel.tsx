...
export function FrameworkPanel({ profile, summary, bundle, onChoose, onSaveCustom }: Props) {
  ...
  return <section className="app-card fw-panel">
    <div className="fw-panel-head"><div><b>{fw.name}</b><small>{custom ? `${fw.origin}${fw.base ? ` · ngân sách ${vnd(fw.base)}/tháng` : income ? ` · theo thu nhập ${vnd(income)}` : ""}` : fw.origin}</small></div>
    ...
    {!income && !fw.base && fw.buckets.some((bucket) => bucket.share !== undefined) && <p className="fw-note">Ghi khoản thu (lương) tháng này để thấy mức mục tiêu của từng phần.</p>}
    <div className="fw-progress">{progress.map((bucket) => {
      ...
      return <div key={bucket.key} className="fw-row">
        <div className="fw-row-top"><b>{bucket.label}{!fw.base && bucket.amount ? ` · ${vnd(bucket.amount)}/tháng` : bucket.share !== undefined ? ` · ${Math.round(bucket.share * 1000) / 10}%` : ""}</b><span>{vnd(bucket.actual)}{bucket.target !== undefined && bucket.target > 0 ? ` / ${vnd(bucket.target)}` : ""}</span></div>
