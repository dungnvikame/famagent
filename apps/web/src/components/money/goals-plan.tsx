  ...
  return <div className="money-settings">
    ...
    <section className="app-section"><h2>Mục tiêu tiết kiệm</h2>
      <div className="app-card app-rows">
        ...
        <div className="inline-form">
          ...
          <AmountInput aria-label="Số tiền mục tiêu" inputMode="decimal" placeholder="Mục tiêu 100tr" value={goal.target} onChange={(event) => setGoal({ ...goal, target: event.target.value })} />
          <AmountInput aria-label="Đã có" inputMode="decimal" placeholder="Đã có (trống = số dư tiết kiệm)" value={goal.saved} onChange={(event) => setGoal({ ...goal, saved: event.target.value })} />
          <AmountInput aria-label="Mỗi tháng" inputMode="decimal" placeholder="Góp/tháng 5tr" value={goal.plan} onChange={(event) => setGoal({ ...goal, plan: event.target.value })} />
    ...
    <section className="app-section"><h2>Kế hoạch chi mỗi tháng</h2><p className="app-sub" style={{ marginBottom: 10 }}>FamAgent so nhịp chi với con số này. Số dư và khoản cố định nằm ở tab Tình hình.</p>
      <div className="app-card inline-form settings-form">
        <label>Kế hoạch chi mỗi tháng<AmountInput inputMode="decimal" placeholder="25tr" value={plan} onChange={(event) => setPlan(event.target.value)} /></label>
