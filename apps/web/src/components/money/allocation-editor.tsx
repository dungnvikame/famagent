...
interface Props {
  ...
  /** This month's income (else the onboarding estimate); 0 = unknown. Only a suggestion for the monthly budget. */
  income: number;
...
/**
 * "Tự thiết kế theo nhà mình": a fixed monthly budget split into named parts (percent and amount, the same every
 * month) with the ledger categories that count toward each; the family can add its own categories here too.
 * Saving needs the shares to add up to 100%.
 */
export function AllocationEditor({ initial, categories: startCategories, income, onSave, onCancel }: Props) {
  // Older splits had no budget: start from this month's income, keeping parts that were set as an amount.
  ...
  async function save() {
    ...
    if (!base) { setError("Nhập ngân sách chia mỗi tháng."); return; }
    if (Math.abs(total - 1) > 0.001) { setError(`Tổng các phần đang là ${pct(total)}%, cần đúng 100%.`); return; }
  ...
  return <div className="alloc">
    ...
    <div className="alloc-base"><label htmlFor="alloc-base">Ngân sách chia mỗi tháng</label>
      <span className="amt-field"><AmountInput id="alloc-base" placeholder="Ví dụ 30.000.000" value={baseText} onChange={(event) => changeBase(event.target.value)} /><span>đ</span></span>
      {income > 0 && income !== base && <small>Thu nhập tháng này {vnd(income)} · <button type="button" className="ledger-link" onClick={() => changeBase(income.toLocaleString("vi-VN"))}>dùng số này</button></small>}
    </div>
    ...
    {buckets.map((bucket, index) => {
      ...
      return <div className="bucket" key={bucket.key}>
        <div className="bucket-top">
          ...
          <span className="amt-field"><AmountInput aria-label={`Số tiền ${bucket.label}`} placeholder="Số tiền" disabled={!base} value={typing?.key === bucket.key ? typing.text : amountText(bucket)} onChange={(event) => { setTyping({ key: bucket.key, text: event.target.value }); setAmount(bucket.key, event.target.value); }} onBlur={() => setTyping(null)} /><span>đ</span></span>
    ...
    <div className="alloc-total">
      ...
      <div className="row"><span>Tổng: <b>{pct(total)}%</b>{base > 0 ? ` · ${vnd(base * total)} / ${vnd(base)}` : ""}</span>{Math.abs(total - 1) <= 0.001 ? <span className="ok-text">✓ Đã chia hết</span> : total < 1 ? <span className="warn-text">Còn {base > 0 ? vnd(base * (1 - total)) : `${pct(1 - total)}%`} chưa chia</span> : <span className="warn-text">Vượt {base > 0 ? vnd(base * (total - 1)) : `${pct(total - 1)}%`}</span>}</div>
    </div>
