...
export function AllocationEditor({ initial, categories: startCategories, income, onSave, onCancel }: Props) {
  const [buckets, setBuckets] = useState<AllocationBucket[]>(initial.buckets);
  ...
  async function save() {
    ...
    if (Math.abs(total - 1) > 0.0005) { setError(`Tổng các phần đang là ${pct(total)}%, cần đúng 100%.`); return; }
    setBusy(true); setError("");
    try { await onSave({ buckets: buckets.map((bucket) => ({ ...bucket, label: bucket.label.trim(), categories: bucket.categories.filter((name) => pool.has(name)) })) }, categories); }
