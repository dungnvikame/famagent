...
export function UpcomingTimeline({ estimates, today, markers = [], notes = {}, actions }: { estimates: ItemEstimate[]; today: string; markers?: TimelineMarker[]; notes?: Record<string, string>; actions: (estimate: ItemEstimate) => React.ReactNode }) {
    ...
    <div className="app-card">
      {due.length ? <>
        ...
        <div className="app-rows timeline-rows">{due.map((estimate) => <div key={estimate.item.id}>
          <span><b>{estimate.item.name}</b><small className={`due ${tone(estimate.daysLeft!)}`}>{estimate.daysLeft === 0 ? "Ước tính đã hết" : `Còn ~${estimate.daysLeft} ngày · hết khoảng ${dayLabel(estimate.runsOutOn!)}`}{estimate.lastPackPrice ? ` · lần trước ${shortVnd(estimate.lastPackPrice)}/gói` : ""}{notes[estimate.item.id] ? ` · ${notes[estimate.item.id]}` : ""}</small></span>
