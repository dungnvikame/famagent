...
export function AgentShopping() {
  ...
  async function send(value = message) {
    ...
    try {
      ...
      const agentTurn: ChatTurn = { id: crypto.randomUUID(), role: "assistant", text: result.text, createdAt: new Date().toISOString(), intent: result.intent, recommendations: result.recommendations, candidateCount: result.candidateCount, candidateProductIds: result.candidateProductIds, rankingVersion: result.rankingVersion, view: result.view, choices: result.choices, notesRecorded: result.notesRecorded?.length ? result.notesRecorded : undefined, purchaseDraft: result.purchaseDraft };
  ...
  function markPurchaseSaved(turnId: string, summary: string, item: ShoppingItem) {
    ...
    const next = conversations.map((conversation) => conversation.id === activeId ? { ...conversation, turns: conversation.turns.map((turn) => turn.id === turnId ? { ...turn, purchaseDraft: undefined, purchaseSaved: summary } : turn) } : conversation);
    persist(next);
