  ...
  async function send(value = message) {
    ...
    try {
      ...
      const result: ChatResponse = moneyQuestion
        ...
        : await fetch("/api/chat", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ message: input, profile, previousIntent: lastIntent, conversationId: activeId, stock: cloudEnabled ? undefined : stock, avoidBrands: cloudEnabled ? undefined : brandsToAvoid(await loadNotes()) }) }).then(async (response) => {
      ...
      const agentTurn: ChatTurn = { id: crypto.randomUUID(), role: "assistant", text: result.text, createdAt: new Date().toISOString(), intent: result.intent, recommendations: result.recommendations, candidateCount: result.candidateCount, candidateProductIds: result.candidateProductIds, rankingVersion: result.rankingVersion, view: result.view, choices: result.choices, notesRecorded: result.notesRecorded?.length ? result.notesRecorded : undefined };
