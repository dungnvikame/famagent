...
export function AgentShopping() {
  ...
  return <div className="agent-app shopping-app"><div className="agent-decoration one" aria-hidden="true"/><div className="agent-decoration two" aria-hidden="true"/>
    ...
    <main className="agent-center shopping-center"><div className="agent-header"><span className="agent-status"><span className="status-dot"/> {mode === "ai" ? "AI đang hỗ trợ" : profile && !profile.aiConsent ? <>Đang dùng quy tắc · <button type="button" className="agent-inline-toggle" onClick={() => updateProfile({ ...profile, aiConsent: true, updatedAt: new Date().toISOString() })}>Bật AI</button></> : "Agent đang sẵn sàng"}</span><span className="agent-header-actions"><Link className="agent-history-trigger" href="/onboarding?update=1">Cập nhật hồ sơ</Link> <button className="agent-history-trigger" onClick={() => void send("Cho tôi xem lịch sử trò chuyện")}>Lịch sử ↗</button></span></div>{contextChips.length > 0 && <div className="agent-context" aria-label="Thông tin agent đang dùng">{contextChips.map((chip) => <span key={chip}>{chip}</span>)}</div>}
