...
export function AgentShopping() {
    ...
    try {
      ...
      const result: ChatResponse = big
        ...
        : await fetch("/api/chat", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ message: input, profile, previousIntent: lastIntent, conversationId: activeId, stock: cloudEnabled ? undefined : stock, items: cloudEnabled ? undefined : shoppingItems, avoidBrands: cloudEnabled ? undefined : brandsToAvoid(await loadNotes()) }) }).then(async (response) => {
          ...
          // Demo mode records notes here (signed-in users get them from the server).
          return cloudEnabled ? data : { ...data, notesRecorded: recordLocalNotes(extractNotes(input, profile, [...new Set(products.map((product) => product.brand))]), activeId) };
        });
      setMode(result.mode);
      if (result.profile) { setProfile(result.profile); saveProfile(result.profile); trackEvent("family_profile_updated"); }
      const agentTurn: ChatTurn = { id: crypto.randomUUID(), role: "assistant", text: result.text, createdAt: new Date().toISOString(), intent: result.intent, recommendations: result.recommendations, candidateCount: result.candidateCount, candidateProductIds: result.candidateProductIds, rankingVersion: result.rankingVersion, view: result.view, choices: result.choices, notesRecorded: result.notesRecorded?.length ? result.notesRecorded : undefined, purchaseDraft: result.purchaseDraft, decision: result.decision };
      if (result.notesRecorded?.length) trackEvent("family_note_recorded", { count: result.notesRecorded.length });
  ...
  /** A confirmed chat draft becomes a note on the turn (never shown as a card again) and joins the item list. */
  function markPurchaseSaved(turnId: string, summary: string, item: ShoppingItem) {
    setShoppingItems((current) => [...current.filter((entry) => entry.id !== item.id), item]);
    markTurn(turnId, { purchaseDraft: undefined, purchaseSaved: summary });
  }

  /** Records what happened to a card on a turn (saved purchase, decision taken) so reopening never offers it again. */
  function markTurn(turnId: string, patch: Partial<ChatTurn>) {
    const next = conversations.map((conversation) => conversation.id === activeId ? { ...conversation, turns: conversation.turns.map((turn) => turn.id === turnId ? { ...turn, ...patch } : turn) } : conversation);
    persist(next);
    const current = next.find((conversation) => conversation.id === activeId);
    if (cloudEnabled && current) void saveCloudConversation(current).catch(() => setError("Chưa lưu được cuộc trò chuyện."));
  }
