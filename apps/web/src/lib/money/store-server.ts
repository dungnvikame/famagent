...
export const budgetFromRow = (row: Row): MoneyBudget => ({ id: row.id as string, category: row.category as string, month: (row.month as string).slice(0, 7), limitAmount: num(row.limit_amount) });
...
export const recurringFromRow = (row: Row): MoneyRecurring => ({ id: row.id as string, name: row.name as string, category: row.category as string, kind: row.kind as MoneyRecurring["kind"], amount: num(row.amount), dayOfMonth: num(row.day_of_month), active: row.active !== false, lastPostedMonth: str(row.last_posted_month)?.slice(0, 7) });
...
export const goalFromRow = (row: Row): MoneyGoal => ({ id: row.id as string, name: row.name as string, targetAmount: num(row.target_amount), savedAmount: num(row.saved_amount), monthlyPlan: row.monthly_plan === null || row.monthly_plan === undefined ? undefined : num(row.monthly_plan) });
