...
const currentAllocation = (allocation?: MoneySettings["allocation"] | null): MoneySettings["allocation"] => allocation ? { buckets: allocation.buckets.map((bucket) => ({ ...bucket, categories: [...new Set(bucket.categories.map((name) => currentCategory(currentCategory(name, "expense"), "saving")))] })) } : undefined;
