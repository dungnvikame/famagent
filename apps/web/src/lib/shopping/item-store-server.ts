...
export const planFromRow = (row: Row): PlanEntry => ({ id: row.id as string, month: row.month as string, itemId: str(row.item_id), stageKey: str(row.stage_key), name: row.name as string, packs: Number(row.packs) || 1, estAmount: num(row.est_amount), reason: row.reason as PlanEntry["reason"], status: row.status as PlanEntry["status"] });
