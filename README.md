...
5. Nếu muốn agent dùng OpenAI, điền `OPENAI_API_KEY`, `OPENAI_MODEL` và đặt `AI_ENABLED=true` phía server. Người dùng phải bật đồng ý trong onboarding. Khi thiếu cấu hình hoặc chưa đồng ý, ứng dụng dùng quy tắc để thử luồng.
...
- `apps/web/src/lib/ai`: Onboarding Agent, Intent Agent, ghép bối cảnh, giải thích và adapter OpenAI.
